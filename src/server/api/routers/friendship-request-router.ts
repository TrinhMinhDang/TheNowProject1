  ...
  accept: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      await ctx.db.transaction().execute(async (t) => {
        ...
        /*return await t.insertInto('friendships')
        .values({
          userId: ctx.session.userId,
          friendUserId: input.friendUserId,
          status: FriendshipStatusSchema.Values['accepted'],
        ...
        .onConflict((oc)=> oc
        .column('friendUserId')
        .doUpdateSet({status: FriendshipStatusSchema.Values['accepted']})
        )
        //.onDuplicateKeyUpdate({status: FriendshipStatusSchema.Values['accepted']})
        .executeTakeFirst()*/
        return await t.replaceInto('friendships')
        .values({
          userId: ctx.session.userId,
          friendUserId: input.friendUserId,
          status: FriendshipStatusSchema.Values['accepted'],
